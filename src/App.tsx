...
const DEFAULT_FOLDER = '12NUdzjgOj8lyyV98h1wmcIm44UTQRDNQ';
...
function App() {
  ...
  function calculate(){
    ...
    setPlan(buildPlan({fbs,sales,fbw,catalog:mergedCatalog,includedWarehouses:included,boxes,accepted:requests,settings:plannerSettings}));setTab('planning');
    alert('ok','Черновик пересчитан. Ничего не сохранено и не зарезервировано.');
  }
  ...
  async function acceptPlan(){
    ...
    try{
      ...
      const now=new Date(); const slug=now.toISOString().slice(0,16).replace(/[-:T]/g,''); const xlsx=requestXlsx(selectedRows); const pdf=await requestPdf(selectedRows);
      const xlsxName=`FBS_${slug}.xlsx`,pdfName=`FBS_${slug}.pdf`; let files:AcceptedRequest['files'];
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {tab==='reports'&&<section className="page reports-page">
        ...
        <div className="next-panel"><div><b>{reportReady?'Отчёты готовы к расчёту':'Загрузите недостающие отчёты'}</b><span>{reportReady?'Можно перейти к планированию. Черновик не меняет остатки.':'Для точного расчёта нужны все три файла.'}</span></div><button className="primary" disabled={!reportReady||!!busy} onClick={calculate}><Play size={18}/> Запустить планирование</button></div>
      </section>}
