...
import type { AcceptedRequest, CatalogItem, FbsItem, FbwItem, PhysicalBox, PlanRow, PlannerSettings, SalesItem, WarehouseIssue } from './types';
import './styles.css';
import './errors.css';
...
function App() {
  ...
  async function calculate(){
    ...
    alert('ok',`Использовано коробок: ${currentBoxes.length}. Черновик пересчитан, ничего не сохранено.`);
  }
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {tab==='errors'&&<section className="page errors-page"><div className="section-title"><div><h2>Ошибки склада</h2><p>Проверьте коробку и разрешите её вручную, если фактический состав верный.</p></div><button className="secondary" disabled={busy==='google'} onClick={()=>void connect().catch(()=>{})}><RefreshCw size={17}/>{busy==='google'?'Обновляем…':'Обновить из Google'}</button></div>
        {!token&&!boxIssues.length?<div className="empty"><AlertTriangle size={34}/><h3>Данные склада ещё не загружены</h3><p>Подключите Google, чтобы проверить BOX_ID, состав, количество, статус и место хранения.</p><button className="primary" onClick={()=>void connect().catch(()=>{})}><Cloud size={17}/> Подключить Google</button></div>:!boxIssues.length?<div className="empty warehouse-ok"><ShieldCheck size={34}/><h3>Ошибок склада не найдено</h3><p>{boxes.length} коробок готовы к автоматическому планированию.</p></div>:<><div className="error-summary"><Metric label="Готовы автоматически" value={boxes.length} note="без замечаний"/><Metric label="Требуют проверки" value={unresolvedIssues.length} note="не участвуют в расчёте"/><Metric label="Разрешены вручную" value={approvedIssueBoxes.length} note="участвуют после пересчёта"/></div><div className="warehouse-issues">{boxIssues.map(issue=>{const approved=confirmedBoxKeys.includes(issue.key);return <article key={issue.key} className={approved?'approved':''}><div className="issue-head"><div><span className={`status ${approved?'ok':'warning'}`}>{approved?'Разрешена':'Нужна проверка'}</span><h3>{issue.box.id||'Коробка без BOX_ID'}</h3><p>{[issue.box.type,issue.box.palette,issue.box.placement,issue.box.storageCells].filter(Boolean).join(' · ')||'Место не указано'} · {issue.box.totalQty} шт.</p></div><button className={approved?'secondary':'primary'} disabled={!issue.confirmable} onClick={()=>toggleBoxConfirmation(issue.key)}>{approved?<><RotateCcw size={16}/> Отменить разрешение</>:<><Check size={16}/> Подтвердить и использовать</>}</button></div><ul>{issue.reasons.map(reason=><li key={reason}>{reason}</li>)}</ul><details><summary>Показать состав — {issue.box.components.length} позиций</summary><div className="issue-components">{issue.box.components.map((component,index)=><div key={`${component.barcode}-${index}`}><span><b>{component.article||component.barcode}</b><small>{component.barcode} · размер {component.size||'—'}</small></span><strong>{component.qty} шт.</strong></div>)}</div></details>{!issue.confirmable&&<p className="cannot-confirm">Коробку нельзя разрешить: сначала исправьте BOX_ID или состав в таблице хранения.</p>}</article>})}</div></>}
      </section>}
